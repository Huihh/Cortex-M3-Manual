...
为了平稳过度，ARM 建议那些运行在其它 Cortex-M 配置 (profile) 处理器架构上的代码设计应遵从以下规则，并正确设置配置控制寄存器 (CCR: Configuration Control Register)：
...
## 3.6. 独立的监视器 (Exclusive monitor)

Cortex-M3 处理器实现了一个本地独立的监视器，更多信息详见 **ARMv7M ARM Architecture Reference
Manual** 。
...
- 13 个通用目的 (general-purpose) 寄存器， r0 - r12
- 堆栈指针(和备份) 寄存器， SP_process 和 SP_main
- 链接寄存器 (LR: Link Register)，r14
- 程序计数器 (PC: Program Counter)，r15
- 特定目的 (Special-purpose) 程序状态寄存器 (xPSR: Program Status Register)

![](E:\Work\ARM\pictures\Chapter 3\Snipaste_2019-07-09_14-04-55.png)



通用寄存器 r0 - r12 没有特殊的体系结构定义用途。大多数可以指定通用寄存器的指令都可以指定 r0 - r12 。

- **Low registers** ：寄存器 r0 - r7 可由指定通用寄存器的所有指令访问。
- **High registers** ：寄存器 r8 - r12 可由指定通用寄存器的所有 32-bit 指令访问。寄存器 r8 - r12 不能被所有16-bit 指令访问。
- **Stack pointer** ：寄存器 r13 用作堆栈指针 (SP: Stack Pointer) 。SP 忽略 bit[1:0] 位，它自动对齐到字 (4字节对齐) 。**Handler** 模式使用 **SP_main** ，**Thread** 模式可以通过配置使用 **SP_main** 或 **SP_process** 。
- **Link register** ：寄存器 r14 是子程序链接寄存器 (LR: Link Register) 。当执行 **BL** (BL: Branch and Link) 或  **BLX** (BLX: Branch and Link with Exchange) 指令时，LR 用于接收从 PC 返回的地址。**LR** 还用于异常返回。在其他任何时候，可以将 r14 作为通用寄存器使用。
- **Program counter** ：寄存器 r15 是程序计数器 (PC: Program Counter) 。bit[0] 一直为 0，因此指令总是 **字** 或 **半字** 对齐。

更多详细信息详见 **ARMv7-M Architecture Reference Manual** 。



## 3.9. 异常 (Exceptions)

处理器和嵌套向量中断控制器 (NVIC) 优先处理所有异常。当处理异常时：

- 所有的异常都在 **Handle** 模式进行处理。
- 处理器的状态在异常发生时自动入栈保存，在中断服务例程 (ISR: Interrupt Service Rountine) 退出时自动出栈恢复。
- 向量取指与状态保存是并行执行，这样中断响应更高效。

处理器支持咬尾 (tail-chaining) 机制，这样就节省了连续的中断 (back-to-back interrupts) 之间保存和恢复处理器状态而进行的入栈和出栈的开销。



在实现阶段，可以配置处理器支持的中断数量和中断优先级使用的 bit 位数。软件只能在已配置的中断数量和中断优先级 bit 位数的子集中进行选择。

注：向量入口与 ARM 和 Thumb 指令之间的交互兼容。这将导致向量值 bit[0] 位加载到执行程序状态寄存器中(EPSR: Execution Program Status Register) 异常入口的 T-bit 位。因此，需要设置所有向量入口中的 bit[0] 位。创建向量表入口 bit[0] 位为 0，将在与此向量对应的处理程序的第一条指令上生成 **INVSTATE fault** 。



### 3.9.1. 异常处理 (Exception handling)

处理器实现了高效的异常和中断处理，详见 **ARMv7-M Architecture Reference Manual** 。

为了降低中断的延迟，处理器实现了晚到 (late-arrival) 和咬尾 (tail-chaining) 中断机制，在 ARMv7-M 架构中定义：

- 当被访问的内存没有应用等待状态时，从认定中断到执行 ISR 的第一条指令最多有 12 个周期的延迟。要执行的第一个指令的取指与入栈是并行执行的。
- 从中断中返回同样需要 12 个周期，其中返回的指令的取指与出栈是并行执行的。
- 使用零等待状态内存时，咬尾 (Tail chaining) 需要 6 个周期。不进行出栈入栈，只需要取指下一个 ISR 的指令即可。

处理器异常模型除了架构上定义的行为外，还具有以下实现定义的行为：
...
为了最小化中断延迟，处理器放弃任何 **divide** 指令，接受任何挂起的中断。从中断处理程序返回时，处理器从一开始就重新启动 divide 指令，处理器实现中断可连续指令域。多次加载 (LDM: Load Multiple) 操作和多次存储 (Store Multiple) 操作时中断可连续的。EPSR 寄存器的 ICI 字段保存着从中断发生时开始继续加载或存储多个请求所需的信息。

这意味着软件不能使用多次加载 (load-multiple) 或 多次存储 (store-multiple) 指令来访问对读敏感 (read-sensitive) 或对重复写敏感 (sensitive to repeated writes) 的设备或内存区域。在重复读写 (repeated reads or writes) 可能导致不一致的结果或不必要的副作用的情况下，软件不能使用这些指令。



在 LDM 和 STM 操作中的基本寄存器更新有以下几种情况：

- 当指令指定基寄存器写回时，基寄存器更改为已更新的地址。终止恢复原始值。
- 当基寄存器位于 LDM 的寄存器列表中，而不是列表中的最后一个寄存器时，基寄存器将更改为加载的值。

遇到以下情况时，LDM 或 STM 将重启：

- 指令错误 (instruction faults)
- 指令在 IT 里面

如果 LDM 完成基本的加载，则在基本加载之前继续。(If an LDM completed a base load, it is continued from before the base load) 。